...
def comb_mask(paths_name, probs, limit=None, drop_duplicates=True, data_name=None):
    ...
    dfinp['substs_probs'] = dfinp.substs_probs.apply(lambda sp: [(prob * probs[cnt_mask], s) for prob, s in sp if cnt_subword(s) == cnt_mask])
    
    for path in paths_name[1:]:
        dfinp_new = load_substs(path, limit, drop_duplicates, data_name)
        cnt_mask = path.count('<mask>')
        # отсев подстановок с другим количеством subword и домножение вероятности на вероятностью маски
        dfinp_new['substs_probs'] = dfinp_new.substs_probs.apply(lambda sp: [(prob * probs[cnt_mask], s) for prob, s in sp if cnt_subword(s) == cnt_mask])
...
def load_substs(substs_fname, limit=None, drop_duplicates=True, data_name = None, drop_word_groups=True):
    if substs_fname.endswith('&'):
        ...
        return load_substs_(substs_fname, limit, drop_duplicates, data_name, drop_word_groups)

def load_substs_(substs_fname, limit=None, drop_duplicates=True, data_name = None, drop_word_groups=True):
    st = time()
    p = Path(substs_fname)
    ...
    if drop_word_groups:
        dfinp['substs_probs'] = dfinp['substs_probs'].apply(lambda x: [(p, s) for p, s in x if s.startswith(' ') and ' ' not in s.strip()])
...
def sstat(substs_fname, limit=None):
    ...
    print(dfinp.groupby('word').count().to_string())
