...
def evaluate(dataset_fpath=None, df=None, stream=None):
    if df is None:
        assert dataset_fpath is not None, "both df and df_path are none"
        df = read_csv(dataset_fpath, sep='\t', dtype={'gold_sense_id': str, 'predict_sense_id': str})
    else:
        df['gold_sense_id'] = df['gold_sense_id'].apply(str)
        df['predict_sense_id'] = df['predict_sense_id'].apply(str)
    ...
    print('{}\t{}\t{}'.format('word', 'ari', 'count') + endl, file=stream)

    # for word in sorted(words.keys()):
    #     print('{}\t{:.6f}\t{:d}'.format(word, *words[word]) + endl, file=stream)

    print('Weighted ari:\t{:.6f}\t{:d}'.format(weightedari, len(df)) + endl, file=stream)
    print('Mean ari:\t{:.6f}\t{:d}'.format(meanari, len(df)) + endl, file=stream)
